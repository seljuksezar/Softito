...
(4, 'Ali Ozturk', 'ali.ozt(urk@mail.com', '0535-4444444', 'Bursa', 'Nilüfer, 456 Cadde', '2023-04-05 11:20:00', 'Gold', 56780.25, 14),
...
ALTER TABLE urunler
ADD CONSTRAINT fk_kategori_id
FOREIGN KEY (kategori_id)
REFERENCES kategoriler(kategori_id);

ALTER TABLE urunler
ADD CONSTRAINT fk_satici_id
FOREIGN KEY (satici_id) 
REFERENCES saticilar;
...
ADD CONSTRAINT fk_siparis_id
FOREIGN KEY (siparis_id) 
REFERENCES siparisler;

ALTER TABLE siparis_detaylari
ADD CONSTRAINT fk_urun_id
FOREIGN KEY (urun_id) 
REFERENCES urunler;
...
(1, 1, 1, 5, 'Çok hızlı teslim, ürün kusursuz! Kesinlikle tavsiye ederim.', '2024-01-19 08:00:00', 1),
...
(10, 'BIRTHDAY5', 'Doğum günü ayınızda %5 indirim', 5.00, '2024-01-01', '2024-12-31', 3456, 1000),
...
SELECT u.urun_adi, u.stok_sayisi ,k.kategori_adi 
FROM urunler u
...
SELECT siparis_id,durum
FROM siparisler
WHERE durum LIKE '%Teslim Edildi%'

-- 5

SELECT satici_adi,durum
FROM saticilar
WHERE durum LIKE '%Aktif%'
...
SELECT s.siparis_id, kt.teslimat_tarihi
FROM siparisler s
JOIN kargo_tracking kt ON (s.siparis_id = kt.siparis_id)
WHERE kt.teslimat_tarihi >'2024-01-20'

-- 9

SELECT *
FROM odemeler
WHERE durum LIKE '%Basarili%'
...
WHERE m.ad_soyad LIKE '%Ayşe Yilmaz%'
...
SELECT u.urun_adi, u.satilan_adet
FROM urunler u
...
SELECT k.kategori_adi, u.stok_sayisi
FROM urunler u
JOIN kategoriler k ON (u.kategori_id = k.kategori_id)
GROUP BY u.stok_sayisi,k.kategori_adi

-- 15

SELECT u.urun_adi, sd.miktar, sd.birim_fiyati
FROM siparis_detaylari sd
JOIN urunler u ON (u.urun_id = sd.urun_id)

-- 16

SELECT m.ad_soyad, m.siparis_sayisi
FROM musteriler m
WHERE m.siparis_sayisi > 5

--17

SELECT m.ad_soyad, m.uyelik_tipi
FROM musteriler m
WHERE m.uyelik_tipi LIKE '%Gold%'
...
SELECT SUM(1) as Teslim_edilen_siparis_sayisi
FROM kargo_tracking
WHERE durum LIKE '%Teslim Edildi%'
...
SELECT u.urun_adi
FROM urunler u
JOIN favoriler f ON (f.urun_id = u.urun_id)
JOIN musteriler m ON (m.musteri_id = f.favori_id)
WHERE m.ad_soyad LIKE '%Fatma Sahin%'

-- 22

SELECT kullanim_sayisi
FROM promosyonlar
WHERE promo_kodu LIKE '%WELCOME10%'

-- 23

SELECT u.urun_adi ,COUNT(y.puan)
FROM yorumlar y 
...
FROM urunler u
JOIN indirimler i ON (i.urun_id = u.urun_id)
ORDER BY i.indirim_yuzde DESC
LIMIT 3

-- 25

SELECT s.siparis_id, kt.durum
FROM siparisler s
JOIN kargo_tracking kt ON (s.siparis_id = kt.siparis_id)
WHERE kt.durum LIKE '%Yolda%'

-- 26

SELECT m.ad_soyad, u.urun_adi, s.durum, s.son_tutar
FROM musteriler m
JOIN siparisler s ON (s.siparis_id = s.musteri_id)
JOIN siparis_detaylari sd ON (s.siparis_id = sd.siparis_id)
JOIN urunler u ON (sd.urun_id = u.urun_id)
WHERE m.ad_soyad LIKE '%Mehmet Kaya%'

-- 27

SELECT sa.satici_adi, sa.ortalama_puan
FROM saticilar sa
ORDER BY sa.ortalama_puan DESC
LIMIT 1

-- 28

SELECT sa.satici_adi, sa.toplam_satis
FROM saticilar sa
ORDER BY sa.toplam_satis DESC
LIMIT 5
...
SELECT u.urun_adi, AVG(u.fiyat)as ort_fiyat, sum(u.stok_sayisi)as toplam_stok
FROM urunler u
JOIN kategoriler k ON (u.kategori_id = k.kategori_id)
WHERE k.kategori_adi LIKE '%Elektronik%'
GROUP BY u.urun_adi
...
SELECT siparis_tarihi, COUNT(s.siparis_id)
FROM siparisler s
...
SELECT u.urun_adi, COUNT(y.yorum_id)as yorum_sayisi
FROM urunler u
JOIN yorumlar y ON (u.urun_id = y.urun_id)
GROUP BY u.urun_adi
...
SELECT ks.sirket_adi,
	AVG(kt.teslimat_tarihi - kt.kargo_tarihi)
FROM kargo_tracking kt
JOIN kargo_sirketleri ks ON (kt.kargo_id = ks.kargo_id)
...
SELECT 
	m.musteri_id, u.urun_adi,k.kategori_adi,s.son_tutar,
	SUM(s.son_tutar) OVER(ORDER BY s.son_tutar ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) as anlik_toplam
FROM siparisler s
...
ORDER BY s.son_tutar

-- 36

SELECT m.ad_soyad, AVG(s.son_tutar)
FROM musteriler m
JOIN siparisler s ON (m.musteri_id = s.musteri_id)
WHERE s.siparis_tarihi > '2024-03-06 14:45:00'::timestamp - INTERVAL '3 days'
GROUP BY m.ad_soyad

-- 37

WITH k_ort as(
	SELECT k.kategori_adi, AVG(u.fiyat)as ort_fiyat
	FROM urunler u
	JOIN kategoriler k ON (u.kategori_id = k.kategori_id)
	GROUP BY k.kategori_adi,u.urun_id
)
SELECT u.urun_adi,u.fiyat, k_ort.ort_fiyat
FROM k_ort
inner JOIN kategoriler k ON (k.kategori_adi = k_ort.kategori_adi)
inner JOIN urunler u ON (k.kategori_id = u.kategori_id)
WHERE u.fiyat - k_ort.ort_fiyat > 0
GROUP BY u.urun_adi,u.fiyat, k_ort.ort_fiyat

-- 38 

WITH en_cok_harcayanlar as (
	SELECT musteri_id,ad_soyad,toplam_harcama
	FROM musteriler
	ORDER BY toplam_harcama DESC
	LIMIT 3
)
SELECT e.ad_soyad, u.urun_adi, e.toplam_harcama
FROM en_cok_harcayanlar e
JOIN musteriler m ON (e.musteri_id = m.musteri_id)
JOIN siparisler s ON (m.musteri_id = s.musteri_id)
JOIN siparis_detaylari sd ON (sd.siparis_id = s.siparis_id)
JOIN urunler u ON (u.urun_id = sd.urun_id)
ORDER BY m.toplam_harcama DESC
